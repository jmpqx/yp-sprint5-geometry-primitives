...
std::optional<Shape> MakeCircle(const std::vector<double> &v) {
    return RequireSize(v, 3)
        .and_then([&](const auto &v) { return RequirePositive(v[2]); })
        .transform([&](const auto &) { return Circle{{v[0], v[1]}, v[2]}; });
}
...
std::optional<Shape> MakeLine(const std::vector<double> &v) {
    return RequireSize(v, 4).transform([&](const auto &) { return Line{{v[0], v[1]}, {v[2], v[3]}}; });
}
...
std::optional<Shape> MakeTriangle(const std::vector<double> &v) {
    return RequireSize(v, 6).transform(
        [&](const auto &) { return Triangle{{v[0], v[1]}, {v[2], v[3]}, {v[4], v[5]}}; });
}
...
std::optional<Shape> MakeRectangle(const std::vector<double> &v) {
    return RequireSize(v, 4)
        .and_then(
            [](const auto &v) { return RequirePositive(v[2]).and_then([&](auto) { return RequirePositive(v[3]); }); })
        .transform([&](auto) { return Rectangle{{v[0], v[1]}, v[2], v[3]}; });
}
...
std::optional<Shape> MakePolygon(const std::vector<double> &v) {
    return RequireSize(v, 4)
        .and_then([](const auto &v) { return RequirePositive(v[2]); })
        .and_then([&](auto) { return ParseDouble(std::to_string(v[3])); })
        .and_then([](auto sides) { return RequireIntegerAtLeast(sides, 3); })
        .transform([&](auto sides) { return RegularPolygon{{v[0], v[1]}, v[2], sides}; });
}
...
std::vector<std::pair<Shape, Shape>> FindAllCollisions(std::span<const Shape> shapes) {
    ...
    auto indices = std::views::iota(size_t{0}, shapes.size());

    std::ranges::for_each(indices, [&](auto i) {
        std::ranges::for_each(std::views::iota(i + 1, shapes.size()), [&](auto j) {
...
std::optional<size_t> FindHighestShape(std::span<const Shape> shapes) {
    ...
    auto heights = std::views::iota(size_t{0}, shapes.size()) |
                   std::views::transform([&](size_t i) { return std::pair{i, queries::GetHeight(shapes[i])}; });
